...
class BudgetGUI:
    def __init__(self, monthly_net_income, mortgage_and_debt, utilities, period="Monthly"):
        """
        Initialize the BudgetGUI with necessary parameters.

        :param monthly_net_income: An instance of the MonthlyNetIncome class
        :param mortgage_and_debt: An instance of the MortgageAndDebt class
        :param utilities: An instance of the Utilities class
        :param period: The period for the budget (default is "Monthly")
        """
        self.root = tk.Tk()  # Create the main window
        self.root.title("Budget GUI")  # Set the window title
        self.monthly_net_income = monthly_net_income  # Store the monthly net income instance
        self.mortgage_and_debt = mortgage_and_debt  # Store the mortgage and debt instance
        self.utilities = utilities  # Store the utilities instance
        self.tooltip = None  # Initialize tooltip to None
        self.period = period  # Set the budget period
    ...
    def create_widgets(self):
        ...
        # Bind the motion event to display tooltips
    ...
    def create_pie_chart(self, frame):
        ...
        # Combine all taxes into one segment
        total_taxes = (
        ...
        # Store original data for restoring later
        ...
        self.canvas = FigureCanvasTkAgg(fig, master=frame)  # Ensure canvas is properly created here
        self.canvas.draw()
        self.canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)

    def on_hover(self, event):
        """
        Display a tooltip and update the pie chart when hovering over a pie chart segment.

        :param event: The event triggered by mouse movement
        """
    ...
    def expand_taxes(self):
        """Expand the Taxes slice into its individual components."""
    ...
    def expand_mortgage_and_debt(self):
        """Expand the Mortgage and Debt slice into its individual components."""
    ...
    def expand_utilities(self):
        """Expand the Utilities slice into its individual components."""
    ...
    def expand_free_money(self):
        """Expand the Free Money slice into its individual components."""
    ...
    def restore_pie_chart(self):
        """Restore the original pie chart data."""
    ...
    def show_tooltip(self, event, index):
        """
        Show a tooltip near the mouse pointer.

        :param event: The event triggered by mouse movement
        :param index: The index of the pie chart segment
        """
        if self.tooltip:
            self.tooltip.destroy()
...
# Create an instance of the BudgetGUI class and run the GUI
